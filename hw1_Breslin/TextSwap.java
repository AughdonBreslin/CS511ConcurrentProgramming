...
public class TextSwap {
    ...
    private static Interval[] getIntervals(int numChunks, int chunkSize) {
        ...
        Interval[] ints = new Interval[numChunks];
        // For each chunk, append a pair from (x, x+c) 
        // where x is the start and x+c is the end
        for(int i = 0; i < numChunks; i++) {
            Interval x = new Interval(chunkSize*i, chunkSize*i+chunkSize);
            ints[i] = x;
        }
    ...
    private static char[] runSwapper(String content, int chunkSize, int numChunks) {
        ...
        char[] buf = new char[chunkSize*numChunks];
        Thread[] groupedBobs = new Thread[numChunks];
        //TO: Order the intervals properly, then run the Swapper instances.
        for (Character label : labels) {
            //Label is char from a to e ascii values 97 to 101, label - 97 = 0 to 4
            int idx = label-97;
            // Offset = index * chunksize
            int off = (idx)*chunkSize;
            Thread bob = new Thread(new Swapper(
                intervals[idx], content, buf, off));
            groupedBobs[idx] = bob;
            bob.start();
        }
        int count = 0;
        while(count != numChunks){
            for (Thread thread : groupedBobs) {
                try {
                    thread.join();
                    count++;
