...
do_join(State, Ref, ChatName) ->
    io:format("client:do_join(...):~n"),
	%% Step 3.2.2: Check if chat is among the client state's connected channels
	case maps:is_key(ChatName, State#cl_st.con_ch) of
		%% Already in chatroom, send error back to GUI, done
    	true -> {err, State};
		%% Client is not in the chatroom
		false -> %% Step 3.2.3: Client sends message to server, 
					%% asking to connect to ChatName
				 Serv = whereis(server),
				 Serv!{self(),Ref,join,ChatName},
				 % Server needs to receive message from client

				 %% Step 3.2.9: Fine-looking client receives message from chatroom
				 receive
...
do_leave(State, Ref, ChatName) ->
    %% Step 3.3.2: Check that ChatName is in the chatroom
	case maps:is_key(ChatName, State#cl_st.con_ch) of
		%% Client is found in chatroom
		true -> %% Step 3.3.3: Client sends message to server 
				Serv = whereis(server),
				Serv!{self(),connect,ChatName},
				% server needs to lookup chatroom PID

				%% Receive message from server
				receive
					%% Step 3.3.8: Remove chatroom from list
					{Serv, Ref, ack_leave} -> %% Step 3.3.9: Send result back to GUI
											  {ok, #cl_st {
												gui = State#cl_st.gui,
												nick = State#cl_st.nick,
		...
		%% Client is not found in chatroom
		false -> {err, State}
	end.
