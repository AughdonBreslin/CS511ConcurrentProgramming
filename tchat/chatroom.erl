...
do_register(State, Ref, ClientPID, ClientNick) ->
	%Step 8: Recieve message then send message to client, then update records of registrations
...
do_unregister(State, ClientPID) ->
    %Step 6a: Remove client from registrations
	#chat_st {
    	name = State#chat_st.name,
...
do_propegate_message(State, Ref, ClientPID, Message) ->
    %Step 4.1: Send message back to sending client
	ClientPID ! {self(), Ref, ack_msg},
	%Step 1.2: Send message to each recipients
	Recipients = maps:remove(ClientPID, State#chat_st.registrations),
	CliNick = maps:get(ClientPID, State#chat_st.registrations),
	Fun = fun(K,_) -> K ! {request, self(), Ref, {incoming_msg, CliNick, State#chat_st.name, Message}} end,
	maps:foreach(Fun, Recipients),
	%Step 2.2: Update chat history
